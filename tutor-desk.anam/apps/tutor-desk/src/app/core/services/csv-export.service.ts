...
export class CsvExportService {
  ...
  exportSubmissionReport(data: CsvSubmissionExportData): void {
    ...
    if (student.className) rows.push(['Class', `${student.className}${student.section ? ` - ${student.section}` : ''}`]);
    rows.push([]);
    ...
    rows.push(['Submitted', submission.submittedAt ? this.formatDate(submission.submittedAt) : 'In Progress']);
    ...
    if (options.includeQuestionDetails && questions.length > 0) {
      ...
      const headers = ['Q#', 'Question', 'Status', 'Marks Obtained', 'Max Marks'];
    ...
    const filename = `${this.sanitizeFilename(student.user.fullName)}_${this.sanitizeFilename(exam.title)}_report.csv`;
  ...
  exportBulkResults(data: CsvBulkExportData): void {
    ...
    if (examTitle || subjectName) {
      rows.push([`Results Report${examTitle ? ` - ${examTitle}` : ''}${subjectName ? ` (${subjectName})` : ''}`]);
    ...
    results.forEach(r => {
      ...
      if (hasTimeSpent) {
        row.push(r.timeSpentSeconds !== undefined ? this.formatTime(r.timeSpentSeconds) : '-');
      }
    ...
    else if (subjectName) filename = this.sanitizeFilename(subjectName) + '_results';
