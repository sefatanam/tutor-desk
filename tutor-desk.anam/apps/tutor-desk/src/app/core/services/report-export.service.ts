...
export class ReportExportService {
  ...
  exportExamReport(data: ExamReportData, options: ReportExportOptions = {}): void {
    console.log('[ReportExportService] exportExamReport called with:', data.examTitle, 'students:', data.students.length);
    const sortedStudents = this.sortStudents(data.students, options);
    const rankedStudents = options.includeRanking !== false 
...
<body>
  ...
  <div class="header">
    ...
    <div class="subtitle">
      ${data.subjectName ? `<span class="subject-badge" style="background: ${data.subjectColor ?? '#6b7280'}">${this.escapeHtml(data.subjectName)}</span>` : '<span class="subject-badge" style="background: #6b7280">Independent Exam</span>'}
  ...
  <div class="table-section">
    ...
    <table class="results-table">
      <thead>
        <tr>
          ${options.includeRanking !== false ? '<th class="center">Rank</th>' : ''}
          <th>Student Name</th>
          <th>Roll No.</th>
          <th>Class</th>
      ...
      <tbody>
        ...
        <tr class="${i % 2 === 0 ? 'even' : 'odd'}">
          ${options.includeRanking !== false ? `<td class="center rank">${s.rank ?? '-'}</td>` : ''}
          <td>
          ...
          <td class="center">
            <span class="result-badge ${s.passed ? 'passed' : 'failed'}">${s.passed ? 'PASS' : 'FAIL'}</span>
          </td>
          <td class="center">
            ${s.hasRetake ? `<span class="retake-badge">#${s.attemptNumber}</span>` : `#${s.attemptNumber}`}
          </td>
        </tr>
        `).join('')}
      </tbody>
  ...
  <!-- Footer -->
  <div class="footer">
    <p>Generated on ${this.formatDate(new Date())} | Tutor Desk - Empowering Education</p>
  ...
  exportSubjectReport(data: SubjectReportData, options: ReportExportOptions = {}): void {
...
<body>
  ...
  <div class="header">
    ...
    <div class="subtitle">
      <span class="subject-badge" style="background: ${data.subjectColor}">${this.escapeHtml(data.subjectName)}</span>
      <span class="separator">|</span>
  ...
  <div class="stats-section">
    ...
    <div class="stats-grid">
      ...
      <div class="stat-box passed">
        <div class="stat-value">${subjectStats.overallPassRate.toFixed(1)}%</div>
        <div class="stat-label">Overall Pass Rate</div>
      </div>
      <div class="stat-box">
        <div class="stat-value">${subjectStats.overallAveragePercentage.toFixed(1)}%</div>
    ...
    <table class="results-table student-exams-table">
      <thead>
        <tr>
          <th>Student Name</th>
          <th>Roll No.</th>
          ${data.exams.map(e => `<th class="center" title="${this.escapeHtml(e.examTitle)}">${this.truncateText(e.examTitle, 12)}</th>`).join('')}
      ...
      <tbody>
        ...
        <tr class="${i % 2 === 0 ? 'even' : 'odd'}">
          ...
          <td>${s.rollNumber ?? '-'}</td>
          ${data.exams.map(exam => {
            const result = s.examResults.find(r => r.examId === exam.examId);
            if (!result || result.score === null) {
              return '<td class="center exam-score not-taken">-</td>';
            }
            const className = result.passed ? 'passed' : 'failed';
            const retakeIndicator = result.hasRetake ? `<sup title="Attempt #${result.attemptNumber}">(${result.attemptNumber})</sup>` : '';
            return `<td class="center exam-score ${className}">${result.score}${retakeIndicator}</td>`;
          }).join('')}
          <td class="center overall-cell">${s.overallPercentage.toFixed(1)}%</td>
  ...
  private sortStudents(
    ...
    sorted.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'roll':
          return multiplier * ((a.rollNumber ?? '').localeCompare(b.rollNumber ?? ''));
        case 'percentage':
  ...
  private calculateSubjectStats(data: SubjectReportData): {
    ...
    return {
      totalSubmissions,
      overallPassRate: submissionCount > 0 ? (totalPassed / submissionCount) * 100 : 0,
      overallAveragePercentage: submissionCount > 0 ? totalPercentage / submissionCount : 0,
      totalRetakes,
    };
  }
