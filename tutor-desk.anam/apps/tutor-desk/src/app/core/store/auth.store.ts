...
import { AuthService, AuthUser, CreateStudentData } from '../services/auth.service';
...
export class AuthStore {
  ...
  async createStudent(
    ...
    try {
      const response = await firstValueFrom(this.authService.createStudent(studentData));
  ...
  async resetUserPassword(
    ...
    try {
      const response = await firstValueFrom(this.authService.resetPassword(userId, newPassword));
  ...
  getDashboardRoute(): string {
    ...
    switch (role) {
      ...
      case 'teacher':
        return this.isPendingApproval() ? '/auth/pending' : '/teacher/dashboard';
      case 'student':
  ...
  private setupTokenRefresh(): void {
    ...
    interval(TOKEN_REFRESH_INTERVAL_MS)
      .pipe(
        ...
        filter(() => this.isAuthenticated() && this.authService.isTokenExpired())
      )
      .subscribe(() => {
        this.refreshSession();
      });
