    ...
    <div class="submission-detail-page">
      ...
      @if (loading()) {
        ...
        <header class="page-header">
          <div class="header-info">
            <div class="student-section">
              <div class="student-avatar" [style.background]="getAvatarColor(student()?.user?.fullName ?? 'S')">
                {{ getInitials(student()?.user?.fullName ?? 'Student') }}
              </div>
              <div class="student-details">
                <h1 class="student-name">{{ student()?.user?.fullName ?? 'Unknown Student' }}</h1>
          ...
          <div class="info-grid">
            ...
            @if (submission()!.autoSubmitReason) {
              <div class="info-item full-width">
                ...
                <span class="info-value warning">{{ submission()!.autoSubmitReason }}</span>
              </div>
            }
          </div>
        </p-card>
        ...
        <p-card styleClass="questions-card">
          <ng-template #header>
            <div class="card-header">
              <h3><i class="pi pi-list-check"></i> Question-by-Question Review</h3>
                ...
                <div class="options-list">
                  @for (option of item.question.options; track option.id) {
                    <div
                      ...
                      [class.wrong]="item.selectedOption?.id === option.id && item.correctOption.id !== option.id"
                    >
...
export class SubmissionDetailComponent implements OnInit {
  ...
  confirmAllowRetake(): void {
    ...
    this.confirmationService.confirm({
      message: `Allow ${student?.user?.fullName ?? 'this student'} to retake this exam?`,
      header: 'Allow Retake',
      icon: 'pi pi-refresh',
