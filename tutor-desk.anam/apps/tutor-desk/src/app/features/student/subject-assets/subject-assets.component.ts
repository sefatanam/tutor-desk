...
import { Subject, Asset, AssetCommentWithUser, CreateAssetCommentDto } from '../../../core/models';
        ...
        @if (assets().length === 0) {
          <p-card>
            <div class="empty-state">
              ...
              <p>Your teacher hasn't posted any learning materials for this subject yet.</p>
            </div>
          </p-card>
        } @else {
                  ...
                  @if (canComment()) {
                    <div class="comment-input">
                      <p-avatar
                        ...
                        shape="circle"
                        [style]="{ background: getAvatarColor(currentUserName()), color: 'white' }"
                      />
                      ...
                      @for (comment of getAssetComments(asset.id); track comment.id) {
                        <div class="comment">
                          <p-avatar
                            ...
                            shape="circle"
                            [style]="{ background: getAvatarColor(comment.user.fullName), color: 'white' }"
                          />
                          <div class="comment__content">
                            <div class="comment__header">
                              <span class="comment__author">{{ comment.user.fullName }}</span>
                              <span class="comment__role">{{ comment.user.role }}</span>
                              <span class="comment__time">{{ comment.createdAt | date:'MMM d, h:mm a' }}</span>
                            </div>
                                ...
                                @for (reply of comment.replies; track reply.id) {
                                  <div class="comment reply">
                                    <p-avatar
                                      ...
                                      shape="circle"
                                      size="normal"
                                      [style]="{ background: getAvatarColor(reply.user.fullName), color: 'white' }"
                                    />
                                    <div class="comment__content">
                                      <div class="comment__header">
                                        <span class="comment__author">{{ reply.user.fullName }}</span>
                                        <span class="comment__role">{{ reply.user.role }}</span>
                                        <span class="comment__time">{{ reply.createdAt | date:'MMM d, h:mm a' }}</span>
                                      </div>
...
export class SubjectAssetsComponent implements OnInit {
  ...
  readonly currentUserName = computed(() => this.authStore.user()?.fullName ?? 'User');
  ...
  private loadComments(assetId: string): void {
    this.db.comments.getByAsset(assetId).pipe(
      takeUntilDestroyed(this.destroyRef)
    ).subscribe({
      next: (comments) => {
  ...
  postComment(assetId: string): void {
    ...
    this.db.comments.create(dto).pipe(
      takeUntilDestroyed(this.destroyRef)
    ).subscribe({
      next: () => {
  ...
  private deleteComment(commentId: string, assetId: string): void {
    this.db.comments.delete(commentId).pipe(
      takeUntilDestroyed(this.destroyRef)
    ).subscribe({
      next: () => {
        this.loadComments(assetId);
