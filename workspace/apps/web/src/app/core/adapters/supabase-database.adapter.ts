...
const mapDbTeacherToModel = (row: Record<string, unknown>): Teacher => ({
  ...
  showExamResultsImmediately:
    (row['show_exam_results_immediately'] as boolean) ?? false,
...
const mapDbTeacherWithUserToModel = (
...
const mapDbExamToModel = (row: Record<string, unknown>): Exam => ({
  ...
  resultVisibility:
    (row['result_visibility'] as ResultVisibility) ?? 'immediate',
  resultReleaseDate: row['result_release_date']
    ? new Date(row['result_release_date'] as string)
    : null,
...
const mapDbExamWithSubjectToModel = (
  row: Record<string, unknown>
): ExamWithSubject => {
...
const mapDbExamAssignmentToModel = (
  row: Record<string, unknown>
): ExamAssignment => ({
  id: row['id'] as string,
  ...
  availableFrom: row['available_from']
    ? new Date(row['available_from'] as string)
    : null,
...
const mapDbExamSubjectAssignmentToModel = (
  row: Record<string, unknown>
): ExamSubjectAssignment => ({
  id: row['id'] as string,
  examId: row['exam_id'] as string,
...
const mapDbSubmissionToModel = (
  ...
  evaluatedAt: row['evaluated_at']
    ? new Date(row['evaluated_at'] as string)
    : null,
...
const mapDbSubmissionWithDetailsToModel = (
  row: Record<string, unknown>
): ExamSubmissionWithDetails => {
  const examRow = row['exams'] as Record<string, unknown>;
  const answersRows = (row['submission_answers'] ?? []) as Record<
    string,
    unknown
  >[];
  return {
...
const mapDbStudentToModel = (row: Record<string, unknown>): Student => ({
  ...
  dateOfBirth: row['date_of_birth']
    ? new Date(row['date_of_birth'] as string)
    : null,
...
const mapDbStudentWithUserToModel = (
  row: Record<string, unknown>
): StudentWithUser => {
  const userRow = row['users'] as Record<string, unknown>;
  return {
...
const mapDbAssetCommentToModel = (
  row: Record<string, unknown>
): AssetComment => ({
  id: row['id'] as string,
...
const mapDbAssetCommentWithUserToModel = (
  row: Record<string, unknown>
): AssetCommentWithUser => {
  const userRow = row['users'] as Record<string, unknown>;
  return {
...
class SupabaseAuthAdapter implements IAuthAdapter {
  ...
  signUpWithEmail(
    _credentials: LoginCredentials & { fullName: string }
  ...
  signInWithGoogle(): Observable<AuthSession> {
    return throwError(
      () => new Error('Use AuthService for Google authentication')
    );
  }
...
class SupabaseUserAdapter implements IUserAdapter {
  ...
  delete(id: string): Observable<void> {
    return from(this.supabase.from('users').delete().eq('id', id)).pipe(
      map(({ error }) => {
        if (error) throw new Error(error.message);
      })
    );
...
class SupabaseTeacherAdapter implements ITeacherAdapter {
  ...
  ): Observable<PaginatedResponse<TeacherWithUser>> {
  ...
  update(id: string, dto: UpdateTeacherDto): Observable<Teacher> {
    ...
    if (dto.allowStudentComments !== undefined)
      updateData['allow_student_comments'] = dto.allowStudentComments;
    if (dto.showExamResultsImmediately !== undefined)
      updateData['show_exam_results_immediately'] =
        dto.showExamResultsImmediately;
  ...
  approve(id: string, approvedBy: string): Observable<Teacher> {
    ...
    return from(
      ...
      switchMap(({ data, error }) => {
        ...
        ).pipe(map(() => mapDbTeacherToModel(teacher)));
      })
    );
  }
  ...
  disable(id: string): Observable<Teacher> {
    return from(
      this.supabase.from('teachers').select('user_id').eq('id', id).single()
    ).pipe(
      switchMap(({ data, error }) => {
        if (error) throw new Error(error.message);
        return from(
  ...
  delete(id: string): Observable<void> {
    return from(this.supabase.from('teachers').delete().eq('id', id)).pipe(
      map(({ error }) => {
        if (error) throw new Error(error.message);
      })
    );
...
class SupabaseStudentAdapter implements IStudentAdapter {
  ...
  getByTeacher(
    teacherId: string,
    params?: PaginationParams
  ): Observable<PaginatedResponse<StudentWithUser>> {
    const page = params?.page ?? 1;
    const pageSize = params?.pageSize ?? 10;
    ...
    return this.authService
      ...
      .pipe(
        switchMap((response) => {
          if (!response.success) {
            return throwError(
              () => new Error(response.error || 'Failed to create student')
            );
          }
  ...
  update(id: string, dto: UpdateStudentDto): Observable<Student> {
    ...
    if (dto.rollNumber !== undefined)
      updateData['roll_number'] = dto.rollNumber;
    ...
    if (dto.guardianName !== undefined)
      updateData['guardian_name'] = dto.guardianName;
    if (dto.guardianPhone !== undefined)
      updateData['guardian_phone'] = dto.guardianPhone;
    if (dto.address !== undefined) updateData['address'] = dto.address;
    if (dto.dateOfBirth !== undefined)
      updateData['date_of_birth'] =
        dto.dateOfBirth?.toISOString().split('T')[0] ?? null;
  ...
  delete(id: string): Observable<void> {
    ...
    return this.getById(id).pipe(
      switchMap((student) => {
        if (!student) throw new Error('Student not found');
        return from(this.supabase.from('students').delete().eq('id', id)).pipe(
          switchMap(() =>
            from(this.supabase.from('users').delete().eq('id', student.userId))
          ),
          map(({ error }) => {
            if (error) throw new Error(error.message);
          })
...
class SupabaseSubjectAdapter implements ISubjectAdapter {
  ...
  getById(id: string): Observable<Subject | null> {
    return from(
      this.supabase.from('subjects').select('*').eq('id', id).single()
    ).pipe(
      map(({ data, error }) => {
        if (error || !data) return null;
  ...
  delete(id: string): Observable<void> {
    return from(this.supabase.from('subjects').delete().eq('id', id)).pipe(
      map(({ error }) => {
        if (error) throw new Error(error.message);
      })
    );
...
class SupabaseExamAdapter implements IExamAdapter {
  ...
  getById(id: string): Observable<ExamWithSubject | null> {
    return from(
      this.supabase.from('exams').select('*, subjects(*)').eq('id', id).single()
    ).pipe(
      map(({ data, error }) => {
        if (error || !data) return null;
  ...
  getByTeacher(
    teacherId: string,
    params?: PaginationParams
  ): Observable<PaginatedResponse<ExamWithSubject>> {
    const page = params?.page ?? 1;
    const pageSize = params?.pageSize ?? 10;
    const fromIndex = (page - 1) * pageSize;
    const toIndex = fromIndex + pageSize - 1;
  ...
  update(id: string, dto: UpdateExamDto): Observable<Exam> {
    ...
    if (dto.timePerQuestionSeconds !== undefined)
      updateData['time_per_question_seconds'] = dto.timePerQuestionSeconds;
    if (dto.allowSkipReturn !== undefined)
      updateData['allow_skip_return'] = dto.allowSkipReturn;
    if (dto.fullscreenRequired !== undefined)
      updateData['fullscreen_required'] = dto.fullscreenRequired;
    if (dto.autoSubmitOnBlur !== undefined)
      updateData['auto_submit_on_blur'] = dto.autoSubmitOnBlur;
    if (dto.allowRetake !== undefined)
      updateData['allow_retake'] = dto.allowRetake;
    if (dto.maxRetakes !== undefined)
      updateData['max_retakes'] = dto.maxRetakes;
    if (dto.scheduledStart !== undefined)
      updateData['scheduled_start'] = dto.scheduledStart?.toISOString() ?? null;
    if (dto.scheduledEnd !== undefined)
      updateData['scheduled_end'] = dto.scheduledEnd?.toISOString() ?? null;
    if (dto.durationMinutes !== undefined)
      updateData['duration_minutes'] = dto.durationMinutes;
    if (dto.passingMarks !== undefined)
      updateData['passing_marks'] = dto.passingMarks;
    // @REVIEW: Result visibility settings
    if (dto.resultVisibility !== undefined)
      updateData['result_visibility'] = dto.resultVisibility;
    if (dto.resultReleaseDate !== undefined)
      updateData['result_release_date'] =
        dto.resultReleaseDate?.toISOString() ?? null;
    if (dto.isResultReleased !== undefined)
      updateData['is_result_released'] = dto.isResultReleased;
    if (dto.showScore !== undefined) updateData['show_score'] = dto.showScore;
    if (dto.showPercentage !== undefined)
      updateData['show_percentage'] = dto.showPercentage;
    if (dto.showPassFail !== undefined)
      updateData['show_pass_fail'] = dto.showPassFail;
    if (dto.showCorrectAnswers !== undefined)
      updateData['show_correct_answers'] = dto.showCorrectAnswers;
    if (dto.showStudentAnswers !== undefined)
      updateData['show_student_answers'] = dto.showStudentAnswers;
    if (dto.showExplanations !== undefined)
      updateData['show_explanations'] = dto.showExplanations;
    if (dto.showQuestionReview !== undefined)
      updateData['show_question_review'] = dto.showQuestionReview;
    if (dto.showTimeSpent !== undefined)
      updateData['show_time_spent'] = dto.showTimeSpent;
    if (dto.showTeacherRemarks !== undefined)
      updateData['show_teacher_remarks'] = dto.showTeacherRemarks;
    ...
    if (dto.totalTimeLimitMinutes !== undefined)
      updateData['total_time_limit_minutes'] = dto.totalTimeLimitMinutes;
    if (dto.showImmediateFeedback !== undefined)
      updateData['show_immediate_feedback'] = dto.showImmediateFeedback;
    if (dto.shuffleQuestions !== undefined)
      updateData['shuffle_questions'] = dto.shuffleQuestions;
    if (dto.shuffleOptions !== undefined)
      updateData['shuffle_options'] = dto.shuffleOptions;
  ...
  delete(id: string): Observable<void> {
    return from(this.supabase.from('exams').delete().eq('id', id)).pipe(
      map(({ error }) => {
        if (error) throw new Error(error.message);
      })
    );
...
class SupabaseQuestionAdapter implements IQuestionAdapter {
  ...
  getByExam(examId: string): Observable<Question[]> {
    return from(
      ...
      map(({ data, error }) => {
        if (error) throw new Error(error.message);
        return (data ?? []).map((row) =>
          mapDbQuestionToModel(row as Record<string, unknown>)
        );
      })
    );
  }
  ...
  createBulk(dtos: CreateQuestionDto[]): Observable<Question[]> {
    ...
    return from(
      this.supabase.from('questions').insert(insertData).select()
    ).pipe(
      map(({ data, error }) => {
        if (error) throw new Error(error.message);
        return (data ?? []).map((row) =>
      ...
      switchMap((questions) =>
        this.updateExamStats(examId).pipe(map(() => questions))
      )
    );
  }
  ...
  update(id: string, dto: UpdateQuestionDto): Observable<Question> {
    ...
    if (dto.questionText !== undefined)
      updateData['question_text'] = dto.questionText;
    if (dto.questionImageUrl !== undefined)
      updateData['question_image_url'] = dto.questionImageUrl;
    if (dto.options !== undefined) updateData['options'] = dto.options;
    if (dto.correctOptionId !== undefined)
      updateData['correct_option_id'] = dto.correctOptionId;
    if (dto.marks !== undefined) updateData['marks'] = dto.marks;
    if (dto.negativeMarks !== undefined)
      updateData['negative_marks'] = dto.negativeMarks;
    if (dto.timeLimitSeconds !== undefined)
      updateData['time_limit_seconds'] = dto.timeLimitSeconds;
    if (dto.sequenceNumber !== undefined)
      updateData['sequence_number'] = dto.sequenceNumber;
  ...
  delete(id: string): Observable<void> {
    ...
    return from(
      this.supabase.from('questions').select('exam_id').eq('id', id).single()
    ).pipe(
      switchMap(({ data, error }) => {
        if (error) throw new Error(error.message);
  ...
  private updateExamStats(examId: string): Observable<void> {
    return from(
      this.supabase.from('questions').select('marks').eq('exam_id', examId)
    ).pipe(
      switchMap(({ data, error }) => {
        if (error) throw new Error(error.message);
        ...
        const totalMarks = questions.reduce(
          (sum, q) =>
            sum + (((q as Record<string, unknown>)['marks'] as number) ?? 0),
          0
        );
...
class SupabaseSubmissionAdapter implements ISubmissionAdapter {
  ...
  getById(id: string): Observable<ExamSubmissionWithDetails | null> {
    return from(
      ...
      map(({ data, error }) => {
        if (error) return null;
        return mapDbSubmissionWithDetailsToModel(
  ...
  getByExam(
    examId: string,
    params?: PaginationParams
  ): Observable<PaginatedResponse<ExamSubmission>> {
    const page = params?.page ?? 1;
    const pageSize = params?.pageSize ?? 10;
    ...
    return from(
      ...
      map(({ data, count, error }) => {
        ...
        return {
          items: (data ?? []).map((row) =>
            mapDbSubmissionToModel(row as Record<string, unknown>)
          ),
          total: count ?? 0,
          page,
          pageSize,
  ...
  getByStudentAndExam(
    studentId: string,
    examId: string
  ): Observable<ExamSubmission | null> {
    return from(
      this.supabase
        .from('exam_submissions')
        .select('*')
      ...
      switchMap(({ data, error }) => {
        ...
        if (lastSubmission && lastStatus !== 'retake_allowed') {
          ...
          if (lastStatus === 'in_progress') {
            throw new Error(
              'You already have an exam in progress. Please resume it.'
            );
          }
          throw new Error(
            'You have already completed this exam. Contact your teacher to request a retake.'
          );
        }

        // @REVIEW: If there was a previous submission with retake_allowed, update it to mark retake as used
        const updatePreviousSubmission$ =
          lastSubmission && lastStatus === 'retake_allowed'
            ? from(
                this.supabase
                  .from('exam_submissions')
  ...
  updateAnswer(
    id: string,
    dto: UpdateSubmissionAnswerDto
  ): Observable<SubmissionAnswer> {
    const updateData: Record<string, unknown> = {};
    ...
    if (dto.timeSpentSeconds !== undefined)
      updateData['time_spent_seconds'] = dto.timeSpentSeconds;
    if (dto.returnedTo !== undefined)
      updateData['returned_to'] = dto.returnedTo;
  ...
  private evaluateAndSubmit(
    ...
    return from(
      ...
      switchMap(({ data, error }) => {
        ...
        const answers = (submission['submission_answers'] ?? []) as Record<
          string,
          unknown
        >[];
        const exam = submission['exams'] as Record<string, unknown>;
        const questions = (exam['questions'] ?? []) as Record<
          string,
          unknown
        >[];
        ...
        const answerUpdates = answers
          .map((answer) => {
            ...
            const selectedOptionId = answer['selected_option_id'] as
              | string
              | null;
            ...
            const negativeMarks =
              parseFloat(question['negative_marks'] as string) || 0;
        ...
        return from(Promise.all(updatePromises)).pipe(
          switchMap(() => {
            ...
            const percentage =
              totalMarks > 0
                ? Math.round((score / totalMarks) * 100 * 100) / 100
                : 0;
      ...
      switchMap((submission) =>
        this.updateExamStats(submission.examId).pipe(map(() => submission))
      )
    );
  }
...
class SupabaseAssetAdapter implements IAssetAdapter {
  ...
  getById(id: string): Observable<Asset | null> {
    return from(
      this.supabase.from('assets').select('*').eq('id', id).single()
    ).pipe(
      map(({ data, error }) => {
        if (error) {
    ...
    return from(
      ...
      map(({ data, count, error }) => {
        ...
        const items = (data ?? []).map((row) =>
          mapDbAssetToModel(row as Record<string, unknown>)
        );
        const total = count ?? 0;
        return {
          items,
  ...
  update(id: string, dto: UpdateAssetDto): Observable<Asset> {
    ...
    if (dto.externalUrl !== undefined)
      updateData['external_url'] = dto.externalUrl;
  ...
  delete(id: string): Observable<void> {
    ...
    return this.getById(id).pipe(
      switchMap((asset) => {
        ...
        return from(this.supabase.from('assets').delete().eq('id', id)).pipe(
          switchMap(({ error }) => {
            if (error) throw new Error(error.message);
  ...
  uploadFile(
    file: File,
    path: string
  ): Observable<{ url: string; fileName: string; size: number }> {
    const filePath = `${path}/${Date.now()}_${file.name}`;
    return from(
      this.supabase.storage.from('assets').upload(filePath, file, {
        cacheControl: '3600',
        upsert: false,
      })
    ).pipe(
  ...
  deleteFile(path: string): Observable<void> {
    return from(this.supabase.storage.from('assets').remove([path])).pipe(
      map(({ error }) => {
        if (error) throw new Error(error.message);
        return undefined;
      })
...
class SupabaseCommentAdapter implements ICommentAdapter {
    ...
    return from(
      ...
      map(({ data, error }) => {
        ...
        const comments = (data ?? []).map((row) =>
          mapDbAssetCommentWithUserToModel(row as Record<string, unknown>)
        );
  ...
  delete(id: string): Observable<void> {
    return from(
      this.supabase.from('asset_comments').delete().eq('id', id)
    ).pipe(
      map(({ error }) => {
        if (error) throw new Error(error.message);
        return undefined;
...
export class SupabaseExamAssignmentAdapter {
  ...
  getByExamWithDetails(
    examId: string
  ): Observable<ExamAssignmentWithDetails[]> {
    return from(
      this.supabase
        .from('exam_assignments')
        .select(
      ...
      map(({ data, error }) => {
        ...
        return (data as Record<string, unknown>[]).map((row) => ({
          ...
          student: mapDbStudentWithUserToModel(
  ...
  createBulk(
    ...
    return from(
      this.supabase.from('exam_assignments').insert(records).select()
    ).pipe(
      map(({ data, error }) => {
        if (error) throw new Error(error.message);
        return (data as Record<string, unknown>[]).map(
  ...
  update(id: string, dto: UpdateExamAssignmentDto): Observable<ExamAssignment> {
    ...
    if (dto.availableFrom !== undefined)
      updateData['available_from'] = dto.availableFrom?.toISOString() ?? null;
    if (dto.dueDate !== undefined)
      updateData['due_date'] = dto.dueDate?.toISOString() ?? null;
    if (dto.status !== undefined) updateData['status'] = dto.status;
    if (dto.maxAttempts !== undefined)
      updateData['max_attempts'] = dto.maxAttempts;
    if (dto.timeLimitMinutes !== undefined)
      updateData['time_limit_minutes'] = dto.timeLimitMinutes;
  ...
  delete(id: string): Observable<void> {
    return from(
      this.supabase.from('exam_assignments').delete().eq('id', id)
    ).pipe(
      map(({ error }) => {
        if (error) throw new Error(error.message);
      })
  ...
  deleteByExam(examId: string): Observable<void> {
    return from(
      this.supabase.from('exam_assignments').delete().eq('exam_id', examId)
    ).pipe(
      map(({ error }) => {
        if (error) throw new Error(error.message);
      })
...
export class SupabaseExamSubjectAssignmentAdapter {
  ...
  getByExam(examId: string): Observable<ExamSubjectAssignment[]> {
    return from(
      ...
      map(({ data, error }) => {
        if (error) throw new Error(error.message);
        return (data as Record<string, unknown>[]).map(
          mapDbExamSubjectAssignmentToModel
        );
      })
    );
  }
  ...
  getByExamWithDetails(
    examId: string
  ): Observable<ExamSubjectAssignmentWithDetails[]> {
    return from(
      this.supabase
        .from('exam_subject_assignments')
        .select(
      ...
      map(({ data, error }) => {
        ...
        return (data as Record<string, unknown>[]).map((row) => ({
          ...
          exam: mapDbExamToModel(row['exams'] as Record<string, unknown>),
          subject: mapDbSubjectToModel(
            row['subjects'] as Record<string, unknown>
          ),
        }));
      })
    );
  ...
  create(
    dto: CreateExamSubjectAssignmentDto
  ): Observable<ExamSubjectAssignment> {
    return from(
      this.supabase
        .from('exam_subject_assignments')
        .insert({
      ...
      map(({ data, error }) => {
        if (error) throw new Error(error.message);
        return mapDbExamSubjectAssignmentToModel(
          data as Record<string, unknown>
        );
      })
    );
  ...
  update(
    id: string,
    dto: UpdateExamSubjectAssignmentDto
  ): Observable<ExamSubjectAssignment> {
    const updateData: Record<string, unknown> = {};
    if (dto.availableFrom !== undefined)
      updateData['available_from'] = dto.availableFrom?.toISOString() ?? null;
    if (dto.dueDate !== undefined)
      updateData['due_date'] = dto.dueDate?.toISOString() ?? null;
    if (dto.autoAssignStudents !== undefined)
      updateData['auto_assign_students'] = dto.autoAssignStudents;
  ...
  delete(id: string): Observable<void> {
    return from(
      this.supabase.from('exam_subject_assignments').delete().eq('id', id)
    ).pipe(
      map(({ error }) => {
        if (error) throw new Error(error.message);
      })
...
const mapDbSettingsCategoryToModel = (
  row: Record<string, unknown>
): SettingsCategory => ({
  id: row['id'] as string,
  name: row['name'] as string,
...
const mapDbSystemSettingToModel = (
  row: Record<string, unknown>
): SystemSetting => ({
  id: row['id'] as string,
...
export class SupabaseSettingsAdapter {
  ...
  getAllCategories(): Observable<SettingsCategory[]> {
    return from(
      ...
      map(({ data, error }) => {
        if (error) throw new Error(error.message);
        return (data ?? []).map((row) =>
          mapDbSettingsCategoryToModel(row as Record<string, unknown>)
        );
      })
    );
  }
  ...
  deleteCategory(id: string): Observable<void> {
    return from(
      this.supabase.from('settings_categories').delete().eq('id', id)
    ).pipe(
      map(({ error }) => {
        if (error) throw new Error(error.message);
      })
  ...
  getAllSettings(): Observable<SystemSetting[]> {
    return from(
      ...
      map(({ data, error }) => {
        if (error) throw new Error(error.message);
        return (data ?? []).map((row) =>
          mapDbSystemSettingToModel(row as Record<string, unknown>)
        );
      })
    );
  }
  ...
  getSettingByKey(key: string): Observable<SystemSetting | null> {
    return from(
      this.supabase.from('system_settings').select('*').eq('key', key).single()
    ).pipe(
      map(({ data, error }) => {
        if (error) return null;
  ...
  updateSetting(
    ...
    if (dto.isRequired !== undefined)
      updateData['is_required'] = dto.isRequired;
    if (dto.sortOrder !== undefined) updateData['sort_order'] = dto.sortOrder;
  ...
  deleteSetting(id: string): Observable<void> {
    return from(
      this.supabase.from('system_settings').delete().eq('id', id)
    ).pipe(
      map(({ error }) => {
        if (error) throw new Error(error.message);
      })
  ...
  bulkCreateSettings(
    settings: CreateSystemSettingDto[]
  ): Observable<SystemSetting[]> {
    ...
    return from(
      this.supabase.from('system_settings').insert(insertData).select()
    ).pipe(
      map(({ data, error }) => {
        if (error) throw new Error(error.message);
        return (data ?? []).map((row) =>
